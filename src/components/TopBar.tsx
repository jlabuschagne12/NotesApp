...
const TopBar = ({sizeFactor, onSizeFactorChange, cellWidth, onWidthChange}: TopBarProps) => {
  ...
  return (
    <div className="flex flex-col md:flex-row-reverse justify-between items-stretch md:items-center px-10 py-5">
      <div className='flex justify-between mb-4 md:mb-0'>
        <Tabs defaultValue="none" value={sortLabel} onValueChange={sortNotes} className="mr-8">
          <TabsList>
        ...
        <IconTooltip tooltipText="Delete All Notes">
          <Button onClick={deleteAllNotes} variant="destructive" size="icon" disabled={notes.length < 2}>
            <Bomb/>
          </Button>
        </IconTooltip>
      </div>
